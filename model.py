...
def series_to_sl(values, is_covid=False):
    ...
    for start, end in zip(
        range(0, len(input_data) - TIME_STEP), range(TIME_STEP, len(input_data))
    ):
        if is_covid:
    ...
    for start, end in zip(
        range(0, len(label) - PREDICTION_STEP), range(PREDICTION_STEP, len(label))
    ):
...
def build_covid_model(x_train, y_train, batch_size, epochs):
    ...
    covid_model.add(
        LSTM(
            LSTM_OUTPUT_SIZE, input_shape=(covid_data.shape[1:]), return_sequences=True
        )
    )
    ...
    covid_model.fit(
        covid_data, y_train, batch_size=batch_size, epochs=epochs, shuffle=True
    )
    return covid_model
...
def build_price_model(x_train, y_train, batch_size, epochs):
    ...
    price_model.add(
        LSTM(
            LSTM_OUTPUT_SIZE, input_shape=(price_data.shape[1:]), return_sequences=False
        )
    )
    ...
    price_model.fit(
        price_data, y_train, batch_size=batch_size, epochs=epochs, shuffle=True
    )
    return price_model
...
def fit_overall_model(model, price_data, covid_data, epochs):
    ...
    # ipdb.set_trace()
    # fit model
    model.fit([covid_input, price_input], covid_label, epochs=epochs, shuffle=True)
...
def model_prediction(symbol):
    ...
    price_data, covid_data = prepare_data(f"{symbol}_daily.csv", shuffle=False)

    model = build_overall_model(price_data, covid_data, batch_size=10, epochs=200)
    ...
    ipdb.set_trace()
    prediction = covid_data["scaler"].inverse_transform(prediction_full.reshape(-1, 1))
